...
public class DrawListenerner implements ActionListener, MouseListener, MouseMotionListener {
	...
	private Shape shape;// 声明图形对象
	private Shape[] shapeArray;// 声明存储图形对象的数组
...
//		}
	...
	public void mouseReleased(MouseEvent e) {
		...
		if (type.equals("直线")) {
			...
			shape = new ShapeRoundRect(Math.min(x1, x2), Math.min(y1, y2), Math.abs(x1 - x2), Math.abs(y1 - y2), color,
	...
	public void mouseDragged(MouseEvent e) {
		...
		if (type.equals("铅笔")) {
			...
			shape = new ShapeFillRect(Math.min(x1, x2), Math.min(y1, y2), Math.abs(x1 - x2), Math.abs(y1 - y2), color,
					new BasicStroke(1));
			shape.draw((Graphics2D) g);
